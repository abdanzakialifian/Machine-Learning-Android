...
import com.learn.machinelearningandroid.databinding.FragmentQABinding
...
class QAFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        client.loadJsonData()?.let { topics ->
            topicContent = topics.getContents()[args.topicID]
            topicSuggestedQuestions = topics.questions[args.topicID]
        }
    ...
    private fun initBertQAModel() {
        bertQAHelper = BertQAHelper(requireContext(), object : BertQAHelper.ResultAnswerListener {
            ...
            override fun onResults(results: List<QaAnswer>?, inferenceTime: Long) {
                results?.first()?.let { qaAnswer ->
                    chatAdapter.addMessage(Message(qaAnswer.text, false))
